...
const template = ({ name, verificationlink }) => {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
        ...
        <tr>
            <td align="center">
                ...
                <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color: #ffffff; border-radius: 10px; overflow: hidden;">
                    ...
                    <!-- Footer -->
                    <tr>
                        <td align="center" style="padding: 20px; background-color: #f8fafc; border-top: 1px solid #e5e7eb;">
...
const sendVerificationMail = async ({ email, name }) => {
    try {
        const info = await transporter.sendMail({
            from: '"sqi-may-node-class" <sqi-node-class@sqi.com>',
            to: `${email}`,
            subject: "Verification", // subject line
            //   text: "Hello world?", // plain text body
            html: template({ name, verificationlink: "https://sqi.edu.ng/" }), // HTML body
        });
